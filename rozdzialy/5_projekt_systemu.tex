...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \end{scope}
    \end{tikzpicture}
    \caption{Diagram przypadków użycia systemu. Klient realizuje pełny proces zakupowy, administrator zarządza katalogiem produktów; logowanie jest~wspólne dla~obu ról. (źródło: opracowanie własne)}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Maszyna stanów cyklu życia zamówienia. Linią ciągłą oznaczono ścieżkę główną, przerywaną --- przejścia do~stanów terminalnych \code{CANCELLED} i~\code{FAILED}, osiągalne z~każdego stanu nieterminalnego (dla~czytelności pokazano reprezentatywne krawędzie). Stany terminalne wyróżniono szarym tłem. (źródło: opracowanie własne)}
...
Schemat głównej tabeli zamówień przedstawiono na~listingu~\ref{lst:schemat-orders}. Identyfikatory zapisano jako
\code{TEXT}, pozycje zamówienia i~adres dostawy --- w~kolumnach \code{JSONB}, a~pełną historię przejść między stanami
