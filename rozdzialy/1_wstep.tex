...
obszarów internetu. Systemy zamówień stanowią jego fundament przez co muszą sprostać z wieloma wyzwaniami.
Takimi jak zmienne w~czasie obciążnie, jak największa dostępność, krótkie czasy odpowiedzi oraz częstymi zmianami.
...
wprowadza jednocześnie nowe wyzwania: konieczność zarządzania komunikacją międzyusługową, zapewnienia spójności 
...
Niniejsza praca podejmuje się tego zagadnienia w~ujęciu praktycznym. Jej przedmiotem jest wytworzenie kompletnego systemu 
obsługi zamówień zbudowanego w~architekturze mikroserwisowej i~wdrożonego w~chmurze publicznej Amazon Web Services 
z~wykorzystaniem zarządzanej usługi Kubernetes (Amazon EKS). Praca ta obejmuje pełby cylk wytwarzania oprogramowania.
Zaczynając od analizy i projektu systemu. Idąc przez implementuacje aplikacji webowej i jej backendu. Z wytworzeniem
...
Celem pracy jest zaprojektowanie, implementacja oraz wdrożenie w~chmurze publicznej Amazon Web Services systemu 
obsługi zamówień opartego na architekturze mikroserwisowej, a~następnie weryfikacja jego działania w~środowisku 
docelowym. Realizacja tak postawionego celu ma jednocześnie wykazać, że współczesne technologie chmurowe --- zarządzana 
usługa Kubernetes, infrastruktura definiowana jako kod oraz zautomatyzowane procesy integracji i~wdrażania --- pozwalają 
pojedynczemu inżynierowi zbudować system o~właściwościach (skalowalność, obserwowalność, bezpieczeństwo) tradycyjnie 
wymagających pracy całych zespołów utrzymaniowych.

Celem tej pracy jest zaprojektowanie, implementacja oraz wdrożnie w~chmurze Amazon Web Services systemu obsługi
zamówień opartego na architekturze mikroserwisowej. Weryfikacja poprawności działania systemu odbędzie się za pomocą
testów manualnych oraz obciążeniowych. Rezultatem tej pracy będzie wykazanie jak za pomocą wymienioncy technologii
można wytworzyć relatywnie szybko system z dobrą skalowalnością, obserwowalnością oraz bezpieczeństwem.

Cel główny można rozdzielić na:

\begin{itemize}
    \item teoretyczna analiza podstaw architektury mikroserwisowej oraz związanymi z nią technologiami (konteneryzacja, Kubernetes, usługi chmurowe AWS),
    \item zaprojektowanie modelu domenowego, architektury usług oraz kontraktów komunikacyjnych systemu obsługi zamówień, 
    \item implementacja mikroserwisów realizujących obsługę zamówień, katalog produktów oraz proces płatności,
    \item implementacja aplikacji webowej stanowiącej interfejs użytkownika systemu,
    \item budowa kompletnego środowiska uruchomieniowego w~chmurze AWS w~podejściu \emph{infrastructure as code},
    \item przeprowadzenie testów systemu oraz analiza jego działania i~kosztów utrzymania w~środowisku chmurowym.
\end{itemize}
...
Poza zakresem pracy pozostają zagadnienia integracji z~rzeczywistymi operatorami płatności (proces płatności jest symulowany), 
obsługa procesów logistycznych następujących po opłaceniu zamówienia oraz implementacja usług pomocniczych --- takich jak 
powiadomienia czy analityka --- które omówiono jedynie jako kierunki dalszego rozwoju systemu w~rozdziale~\ref{chap:wnioski}.
...
Tematy te zostaną omówione jedynie jako kierunki dalszego rozwoju systemu w~rozdziale~\ref{chap:wnioski}. 
...
System zamówień jest bardzo dobrym tematem wykorzystania architektury mikroserwisowej. Dzieli się ona łatwo na odrębne
tematy biznesowe jak katalog produktów, zamówienia oraz płatności. Pomiędzy nimi występują interakcje synchroniczne
...
i koszty niesie za sobą ruchomienie takiego systemu w~chmurze publicznej. Udokumentowani
...
sytuują się pomiedzy modelami IaaS i~PaaS. Dostawcja przyjmuje odpowiedzialność utrzymania platformy uruchomieniowej. Pozostawiając
...
Wszyscy najwięksi dostawcy chmury publicznej oferują zarządzane warianty Kubernetes. Różnice między największymi (Amazon EKS, Google GKE, Azure AKS) 
przedstawiono w~rozdziale~\ref{chap:aws}.
...
Ostatnią grupą są narzędzia wspierające eksploatację systemów rozproszonych. Takie jak systemy monitoringu i~obserowalności.
Brokery komunikatów umożliwiające asynchroniczną wymianę zdarzeń. CI/CD automatyzujące zmiany w repozytorium i środowiku produkcyjnym.

\section{Struktura pracy}
\label{sec:struktura-pracy}

Praca składa się z~jedenastu rozdziałów, które można podzielić na część teoretyczną (rozdziały 2--4), projektową (rozdział 5), 
...
w~tym analizie kosztów wykorzystania usług chmurowych i~identyfikacji wąskich gardeł. Pracę zamyka rozdział~\ref{chap:wnioski}, 
