...
i~stał się standardem przemysłowym w~obszarze orkiestracji \parencite{burns2022}.

U~podstaw działania Kubernetes leży model deklaratywny. Użytkownik zamiast wydawać platformie poleceń, opisuje
...
na których agent \emph{kubelet} uruchamia kontenery zgodnie z~decyzjami planisty \parencite{k8sdocs}. Taka architektura
pozwala usługom chmurowym wziąć odpowiedzialność za płaszczyznę sterwania. Dzięki czemu użytkownik zarządza wyłącznie
...
to musi ją trzymać w zewnętrznym miejscu.
...
\textbf{Service} rozwiązuje problem ulotności podów. Udostępnia stabilny, wirtualny punkt dostępu (nazwę DNS
i~adres IP). Jest to wymagane dla dynamicznie zmieniającego się zbioru replik i zapewnia równoważenie obciążenia pomiędzy nimi.
...
\textbf{ConfigMap i~Secret} przechowują konfigurację oddzieloną od obrazów kontenerów. ConfigMap zawiera jawną,
a~secret poufną. Udostępniają one podom zmienne środowiskowe lub pliki. Obiekty te są infrastrukturalną realizacją 
...
dysponującymi wolnymi zasobami w zadeklarowanej wielkości. Limity natomiast wyznaczają górną granicę zużycia, której 
przekroczenie skutkuje dławieniem procesora lub zakończeniem procesu w przypadku pamięci.
...
Skalowanie obciążeń realizowane jest na dwóch poziomach. Poziom pierwszy stanowi \textbf{Horizontal Pod Autoscaler}
...
Deploymentu w~zadanym przedziale \parencite{k8sdocs}. Poziom drugi to skalowanie samego klastra, czyli liczby węzłów
roboczych.
...
Z tego powodu Kubernetes rozwiązuje ten problem warstwą abstrakcji nad pamięcią trwałą, 
rozdzielającą perspektywę administratora i~aplikacji \parencite{k8sdocs}.

\textbf{PersistentVolume} (PV) reprezentuje fizyczny zasób pamięci. Natomiast \textbf{PersistentVolumeClaim} (PVC) 
jest żądaniem zapotrzebowania aplikacji na taki woluminu o potrzebnej pojemności i~trybu dos†ępu.
...
przez Prometheus \parencite{prometheusdocs}.
...
Rozwiązanie to wykorzystano w~zrealizowanym systemie: każdy mikroserwis opatrzony jest adnotacjami wskazującymi 
...
Bezpieczeństwo klastra Kubernetes obejmuje kilka współdziałających warstw \parencite{k8sdocs}.

Każde żądanie do serwera API podlega uwierzytelnieniu i~autoryzacji w~modelu RBAC (\emph{Role-Based Access Control}).
...
Prometheus. Posiada uprawnienia do dynamicznego odkrywania celów poprzez odczyt metadanych podów.
