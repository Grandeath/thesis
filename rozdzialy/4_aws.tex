...
Infrastruktura AWS jest podzielona \textbf{regiony}. Są to geograficznie odseparowane lokalizacje (np. \code{eu-central-1} we Frankfurcie).
...
Integralną częścią EKS są \textbf{dodatki} (\emph{add-ons}). Zarządzame przez AWS komponenty systemowe klastra.
Przykładem takiej wtyczki może być sieciowa VPC CNI. Przydziela ona podom adresy IP bezpośrednio z~puli
adresowej sieci VPC. Bez niej liczba podów na weźle jest ograniczona liczbą interfejsów sieciowych instacji.
W tym projekcie użyłem małych instacji co stanowiłob bariere przy wdrożeniu wszystkich komponentów.
...
opisane typami instancji. W zrealizowanym systemie użyte zostały jako węzły EKS. Stanowią one wartswe obliczeniową klastra.
W celu zmieszczenia w darmowym programie użyte zostały instacje \code{t3.small} w~trybie \emph{on-demand}.
...
Korzystają z~niej wszystkie 3 mikroserwisy. Instacja umieszczona jest w~podsieci prywatnej. Z~grupą bezpieczeństwa,
...
dla serwisu z skanowaniem obrazów pod kątem podatności. Ustawiono również cykl życia ograniczającą liczbę przechowywanych wersji
obrazó∑. System ten zintegrowany jest z IAM. EKS za pomocą roli tylko do odczytu pobiera podczas deploymentu ostatnią wersje obrazu.
...
pełni w~systemie również role pojedyńczego punktu wejścia dla API który kieruje ruch do load balancera.
Cloudfront cacheuje również statyczne dane na lokalizacjach brzegowych, więc ogranicza on zapytania do S3
...
w zrealizowanym systemie jest podział na podsieci publicze i prywatne. Sieć publiczna ma dostęp do internetu
...
Są to tanowe zapory przypisywane zasobom, zezwalające wyłącznie na jawnie zdefiniowane przepływy.
...
Role to tożsamości przyjmowane tymczasowo przez usługi i procesy. Działają na zasadzie najmnijeszych 
...
AWS Secrets Manager uzupełnia IAM o~scentralizowane przechowywanie sekretów aplikacyjnych jak haseł, kluczy
