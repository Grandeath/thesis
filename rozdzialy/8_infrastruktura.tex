...
\begin{figure}[H]
    ...
    \caption{Reguły zachowań (\emph{behaviors}) CloudFront: ścieżki \code{/api/*} oraz~\code{/grafana*} kierowane są~do~ALB z~wyłączonym buforowaniem, a~pozostałe --- domyślnie do~bucketa S3 z~buforowaniem zoptymalizowanym. (źródło: opracowanie własne)}
...
w~przestrzeni nazw \code{monitoring} przedstawiono na~rysunku~\ref{fig:prometheus-eks}, a~udostępniony interfejs
Grafany z~kokpitami mikroserwisów --- na~rysunku~\ref{fig:grafana}.
