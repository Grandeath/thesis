...
\phantomsection
\addcontentsline{toc}{chapter}{Wnioski i podsumowanie}
\label{chap:wnioski}
...
\phantomsection
\addcontentsline{toc}{section}{Stopień realizacji celów pracy}
...
Realizację celów szczegółowych, postawionych we~wstępie, podsumowuje
...
powiadomień i~analityki jako kierunek dalszego rozwoju. Decyzja ta
...
\phantomsection
\addcontentsline{toc}{section}{Napotkane trudności i sposoby ich rozwiązania}
...
\phantomsection
\addcontentsline{toc}{section}{Rekomendacje dotyczące dalszego rozwoju systemu}
...
\begin{itemize}
    ...
    \item \textbf{Rozdzielenie warstwy danych.} Przydzielenie każdej usłudze własnej bazy oraz użycie realnej
     finalizacji płatności przywróciłoby pełną zgodność ze~wzorcem \emph{database-per-service}.

    \item \textbf{Wysoka dostępność komponentów stanowych.} Pojedynczy broker Kafka oraz~jednostrefowa baza danych
    powinny zostać zastąpione konfiguracjami o wysokiej dostępności (klaster Kafka lub~Amazon~MSK, tryb~Multi-AZ~RDS),
...
\phantomsection
\addcontentsline{toc}{section}{Podsumowanie końcowe}
...
Wszystkie te komponenty uruchomiono na~klastrze Kubernetes w~usłudze Amazon~EKS. Moduł perzystencji danych został postawiony w usłudze
