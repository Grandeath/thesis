...
pojedyńcza jednostka uruchomieniowa. Model ten ma istotne zalety. Jest on prosty koncepcyjnie. Tranzakcje mogą być
...
Problemy architektury monolitycznej ujawniają się wraz ze wzrostem systemu i~organizacji. Skalowanie możliwe
jest poprzez dodanie zasobów lub powielenie całej aplikacji. Nawet jeśli wąskim gardłem jest pojedyńczy moduł.
...
Należy podkreślić, że mikroserwisy nie są rozwiązaniem uniwersalnie lepszym --- przenoszą one złożoność z~wnętrza
aplikacji do przestrzeni pomiędzy usługami: do sieci, infrastruktury i~procesów wdrożeniowych. Wybór tej architektury
jest uzasadniony wówczas, gdy korzyści z~niezależnego skalowania i~wdrażania przewyższają koszt zwiększonej
złożoności operacyjnej; właśnie dlatego praktyczna realizacja mikroserwisów jest dziś nierozerwalnie związana
z~technologiami chmurowymi i~automatyzacją, które ten koszt istotnie redukują.
...
Usługa bierze odpowiedzialność za wyodrębniony fragment domeny. W reazlizowanym systemie ten podział widać na:
zamówieniach, katalog produktów raz płatnościami. Minimalizuje on liczbę zmian wymagających na koordynacji między usługami.
...
mogą być chwilowo niedostępne. Ograniczenie takich awarii można osiągnąc poprzez asynchroniczną komunikacje oraz samonaprawiające się usługi.

\textbf{Obserwowalność.} Zachowanie systemu rozproszonego wymaga dokładnego monitorowania. Z powodu że każda usługa działa całkowicie oddzielnie.
...
protokole HTTP i~formacie JSON: zasoby identyfikowane są adresami URL, a~operacje na nich wyrażane metodami HTTP
(GET, POST, PUT, DELETE). Zaletami podejścia są prostota, powszechna znajomość, łatwość testowania oraz naturalna
zgodność z~modelem żądanie--odpowiedź, właściwym dla interakcji inicjowanych przez użytkownika.
...
pobierają te zdażenia i obsługują je według własnej logiki. Dzięki temu rozszczepieniu serwisów, nadawca nie zna swoich
odbiorców. Powoduje to łatwe dodawanie kolejnych subskrybentów bez zmian po stronie producenta \cite{kleppmann2017}.

Model ten eliminuje sprzężenie czasowe: chwilowa niedostępność konsumenta nie wpływa na producenta, a~zaległe
komunikaty zostaną przetworzone po przywróceniu jego działania. Ceną jest rezygnacja z~natychmiastowej, silnej
spójności danych na rzecz spójności ostatecznej (\emph{eventual consistency}) --- stan rozproszony pomiędzy usługami
uzgadnia się z~opóźnieniem, co projekt systemu musi jawnie uwzględniać. Istotne są również gwarancje dostarczania:
brokery komunikatów typowo zapewniają dostarczenie \emph{co najmniej raz} (\emph{at-least-once}), co oznacza, że ten
sam komunikat może zostać doręczony wielokrotnie --- konsumenci muszą zatem przetwarzać komunikaty w~sposób
idempotentny \cite{kleppmann2017}.
...
serwis zamówień nie dotyka danych produktowych i płatniczych. Wysyłając asynchronicznie rządanie o rezerwacji
produktów.

\textbf{Zdarzenia domenowe i~choreografia.} Proces biznesowy obejmujący wiele usług może być koordynowany
centralnie (orkiestracja) lub realizować się jako łańcuch reakcji na zdarzenia (choreografia). Każda usługa,
...
Realizuje się to poprzez klucz idempotencji. Jest ot unikalny identyfikator operacji przekazywany przez klienta i zapamiętywany przez usługe.
Dzięki tej logice powtórne żądanie nie tworzy duplikatu, a zwraca wynik juz wykonanej operacji.
...
\textbf{Brama API i~odwrócone proxy.} Klient systemu nie kmunikuje się z~usługami bezpośrednio. Istnieje pojedyńczy 
...
Odpytywany jest przez platformę uruchomieniową w~celu wykrywania niezdrowych instacji. Zapbiega to awarii
pprzez automatyczny ich restart lub wyłączenie z~puli.
...
Proocesy izolowane są od siebie przy użyciu jądra Linux, współdzieląc przy tym jądro systemu gospodarza.
...
obrazów. Niemutowalność obrazu zapewnia powtarzalność wdrożeń. Eleminuje to błędy wynikające z~różnic z~środowiskami.

Konteneryzacja rozwiązuje problem pojedyńczej usługi. Jednak nie odpowiada na pytania jak: Na którym węźle uruchomić kontener?
Jak utrzymać zadaną liczbę jego replik mimo awarii węzłów? Jak kierować ruch do zmieniających się dynamicznie instancji? Jak przeprowadzić
aktualizację bez przerwy w~działaniu? Jak skalować liczbę replik w~odpowiedzi na obciążenie. Zadania te są określane jako
orkiestracja kontenerów. Wymaga to specjalnej platformy której standardem stał się Kubernetes, ktremu poświęcono następny rozdział.